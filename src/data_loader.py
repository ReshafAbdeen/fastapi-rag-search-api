...
def load_all_documents(data_dir: str) -> List[Any]:
    """ 
    Load all supported files from the data directory and converter to Lnagchain document structure.
    ...
    print(f"[DEBUG] Loading TXT: {txt_files} TXT files: {[str(f) for f in txt_files]}")
    ...
    for json_file in json_files:
        ...
        try:
            ...
            loaded = loader.load()
            print(f"[DEBUG] Loading {len(loaded)} JSON docs from {json_file}")
            documents.extend(loaded)
        except Exception as e:
            print(f"[ERROR] Failed to load JSON {json_file} : {e}")

    print(f"[DEBUG] Toatal Loaded document: {len(documents)}")
    return documents
